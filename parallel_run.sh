...
for d in ../aws-l0 ../PROMICE-AWS-data-issues; do
  (cd "$d" && git pull)
done
...
df = pd.read_csv(r"$THREDDS_DIR/metadata/AWS_stations_metadata.csv")
...
sites=$(python - <<END
import pandas as pd, numpy as np
df = pd.read_csv(r"$THREDDS_DIR/metadata/AWS_sites_metadata.csv")
...
for site in $sites; do
  tasks+=("site_${site};from test_processing_scripts import get_join_l3; get_join_l3('${site}')")
done
